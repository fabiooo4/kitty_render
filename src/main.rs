...
fn main() {
    ...
    while running.load(Ordering::SeqCst) {
        ...
        let width: usize = winsize.ws_xpixel as usize;
        let height: usize = winsize.ws_ypixel as usize;

        let mut img_buf: Vec<Vec<Color>> =
            vec![vec![Color::try_from("#000000").unwrap(); width]; height];

        (0..height).for_each(|y| {
            (0..width).for_each(|x| {
                let red: f32 = x as f32 / (width as f32 - 1.);
                let green: f32 = y as f32 / (height as f32 - 1.);
